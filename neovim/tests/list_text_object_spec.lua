...
local function test_boundary_detection(content, cursor_search, expected_boundaries)
  ...
  if current_line <= #lines then
    ...
    if string.match(line_content, "^%s*[-*+]%s") or string.match(line_content, "^%s*%d+%.%s") or string.match(line_content, "^%s*-%s*%[.%]%s") then
      ...
      for i = current_line + 1, #lines do
        ...
        elseif string.match(next_line, "^%s*[-*+]%s") or string.match(next_line, "^%s*%d+%.%s") or string.match(next_line, "^%s*-%s*%[.%]%s") then
