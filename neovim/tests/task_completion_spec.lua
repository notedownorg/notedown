...
- [x] Already completed
  ...
  local success, completion_result = pcall(test_utils.lsp_request_sync, "textDocument/completion", completion_params, 3000)
...
- [ ] Need to complete
  ...
  for i, pos in ipairs(test_positions) do
    ...
    if pos.line + 1 > #lines then
      test_utils.cleanup_test_workspace(workspace)
      error("CRITICAL: Test position line " .. pos.line .. " doesn't exist in buffer (only " .. #lines .. " lines)")
    end
    ...
    elseif #items ~= first_completion_count then
      test_utils.cleanup_test_workspace(workspace)
      error("CRITICAL: Inconsistent completion count - first position had " .. first_completion_count .. " items, second position had " .. #items .. " items")
    end
  end
