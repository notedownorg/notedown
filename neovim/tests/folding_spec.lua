...
local function test_list_folding()
  ...
  if #list_ranges > 0 then
    ...
    if task2_range then
      ...
      for _, range in ipairs(list_ranges) do
        if range.startLine == 17 then
          assert_equals(range.endLine > 17, true, 
    ...
    if subtask1_range then
      ...
      assert_equals(subtask1_range.endLine >= 19, true, "Subtask 1 should include nested content (ends at line " .. subtask1_range.endLine .. ")")
    end
    ...
    if item2_range then
      ...
      for _, range in ipairs(list_ranges) do
        if range.startLine == 25 then
          assert_equals(range.endLine > 25, true,
    ...
    if task1_range then
      ...
      assert_equals(task1_range.endLine >= 16, true, "Task 1 range should be valid (ends at line " .. task1_range.endLine .. ")")
    end
    ...
    if item1_range then
      ...
      assert_equals(item1_range.endLine >= 24, true, "Item 1 range should be valid (ends at line " .. item1_range.endLine .. ")")
    end
  else
...
local function test_code_block_folding()
  ...
  if js_code_range then
    ...
    assert_equals(js_code_range.endLine >= 29, true, "JavaScript code block should include function content (ends at line " .. js_code_range.endLine .. ")")
    ...
    if any_code_range then
      assert_equals(any_code_range.startLine >= 27 and any_code_range.startLine <= 31, true,
                    "Code block should start in expected area (found at line " .. any_code_range.startLine .. ")")
      assert_equals(any_code_range.endLine > any_code_range.startLine, true, 
                    "Code block should fold content (lines " .. any_code_range.startLine .. "-" .. any_code_range.endLine .. ")")
...
local function test_manual_fold_operations()
  ...
  if fold_level and fold_level > 0 then
    ...
    test_utils.assert_or_fail(closed_line > 0, "Fold should close properly - got closed_line: " .. tostring(closed_line), {
      expected = "> 0",
      actual = closed_line
    })
    ...
    test_utils.fail("Fold level computation failed - no positive fold level found after " .. max_attempts .. " attempts", {
      expected = "> 0",
