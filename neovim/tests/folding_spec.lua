...
local function test_folding_setup_notedown_files()
	...
	if lsp_ready then
		...
		if filetype == "notedown" then
			...
			assert_equals(foldexpr, "v:lua.require('notedown').notedown_foldexpr()", "Foldexpr should use custom notedown fold expression")
...
local function test_manual_fold_operations()
	...
	assert_equals(foldexpr, "v:lua.require('notedown').notedown_foldexpr()", "Should use custom notedown fold expression")
	assert_equals(foldenable, true, "Folding should be enabled")
