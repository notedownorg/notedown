...
func TestEventBroadcast_Fuzz(t *testing.T) {
	...
	sub1 := make(chan tasks.Event)
	sub2 := make(chan tasks.Event)

	// Listen for events from the tasks client
	got1, got2 := make([]tasks.Operation, 0), make([]tasks.Operation, 0)
	go func() {
		for {
			select {
			case event := <-sub1:
	...
	// Throw some events at the tasks client and ensure we are notified correctly
	want := make([]tasks.Operation, 0)
	count := 1000
	for i := 0; i < count; i++ {
		switch rand.Intn(3) {
		case 0:
			feed <- reader.Event{Op: reader.Load, Key: "test.md"}
			want = append(want, tasks.Load)
		case 1:
			feed <- reader.Event{Op: reader.Change, Key: "test.md"}
			want = append(want, tasks.Change)
		case 2:
			feed <- reader.Event{Op: reader.Delete, Key: "test.md"}
			want = append(want, tasks.Delete)
		}
	}
